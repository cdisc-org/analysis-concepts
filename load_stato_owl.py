...
class OWLToNeo4jLoader:
    ...
    # STATO custom annotation property IRIs
    STATO_ANNOTATIONS = {
        "STATO_0000032": "alternative_term",   # "STATO alternative term"
        "STATO_0000041": "r_command",           # "R command"
        "STATO_0000391": "python_command",      # "Python command"
        "STATO_0000404": "math_notation",       # "preferred mathematical notation"
    }

    def get_annotations(self, entity) -> dict:
        """Extract common annotations and STATO custom annotations from an OWL entity."""
        annotations = {}
        ...
        # STATO custom annotations (R command, Python command, math notation, alt term)
        for attr_name, prop_key in self.STATO_ANNOTATIONS.items():
            if hasattr(entity, attr_name):
                values = list(getattr(entity, attr_name))
                if values:
                    # Filter out empty strings
                    non_empty = [str(v).strip() for v in values if str(v).strip()]
                    if non_empty:
                        annotations[prop_key] = non_empty[0] if len(non_empty) == 1 else non_empty

        return annotations

    def import_classes(self, session):
        """Import all OWL classes including STATO custom annotations."""
        ...
        for cls in self.ontology.classes():
            ...
            # Build STATO ID (e.g., "STATO:0000179") from URI
            stato_id = None
            name = cls.name
            if name and "_" in name:
                prefix, number = name.rsplit("_", 1)
                if number.isdigit():
                    stato_id = f"{prefix}:{number}"

            # Handle list vs single-value for r_command and python_command
            r_command = annotations.get('r_command')
            if isinstance(r_command, list):
                r_command = "\n".join(r_command)
            python_command = annotations.get('python_command')
            if isinstance(python_command, list):
                python_command = "\n".join(python_command)
            alt_term = annotations.get('alternative_term')
            if isinstance(alt_term, list):
                alt_term = "; ".join(alt_term)
            ...
            session.run("""
                ...
                SET c.name = $name,
                    ...
                    c.r_command = $r_command,
                    c.python_command = $python_command,
                    c.math_notation = $math_notation,
                    c.alternative_term = $alternative_term
            """,
                uri=str(cls.iri),
                name=cls.name,
                stato_id=stato_id,
                ...
                math_notation=annotations.get('math_notation'),
        ...
        print(f"  Classes with R command:      {r_cmd_count}")
        print(f"  Classes with Python command:  {py_cmd_count}")
        return count
    ...
    def print_summary(self):
        ...
        with self.driver.session(database=self.database) as session:
            result = session.run("""
                ...
                WITH stato_classes, total_classes, hierarchy, properties, count(i) AS individuals
                OPTIONAL MATCH (r:OWLClass) WHERE r.r_command IS NOT NULL
                WITH stato_classes, total_classes, hierarchy, properties, individuals, count(r) AS with_r_cmd
                OPTIONAL MATCH (py:OWLClass) WHERE py.python_command IS NOT NULL
                RETURN stato_classes, total_classes, hierarchy, properties, individuals, with_r_cmd, count(py) AS with_py_cmd
            ...
            print(f"With R command:      {record['with_r_cmd']}")
            print(f"With Python command: {record['with_py_cmd']}")


def main():
    script_dir = Path(__file__).parent
    stato_dir = script_dir
