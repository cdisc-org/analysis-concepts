...
# Reads metadata from CSV files and executes derivations dynamically
...
get_derivation_info <- function(metadata) {
  ...
  # The formula is in the 'formula' column (e.g., "change_value = analysis_value - baseline_value")
  formula_str <- method_row$formula[1]
  ...
  # Get derivation info from the Derivation row
  derivation_row <- metadata$study_instance %>%
    filter(entity_type == "Derivation")
...
get_cube_adam_mapping <- function(metadata) {
  # Extract mapping from Dimensions and Measures that have data_concept
  # Join with data_concept_mapping to get ADaM_standard_var
  mapping <- metadata$study_instance %>%
    filter(entity_type %in% c("Dimension", "Measure", "Attribute"),
           !is.na(data_concept) & data_concept != "") %>%
    select(name, data_concept) %>%
    distinct() %>%
    left_join(metadata$data_concept_mapping, by = "data_concept") %>%
    select(name, ADaM_standard_var) %>%
    filter(!is.na(ADaM_standard_var))

  # Create named vector: cube_name -> adam_variable
  setNames(mapping$ADaM_standard_var, mapping$name)
}
...
get_dimension_variables <- function(metadata) {
  ...
  dims <- metadata$study_instance %>%
    ...
    distinct() %>%
    left_join(metadata$data_concept_mapping, by = "data_concept") %>%
    select(ADaM_standard_var) %>%
    filter(!is.na(ADaM_standard_var))

  dims$ADaM_standard_var
}
