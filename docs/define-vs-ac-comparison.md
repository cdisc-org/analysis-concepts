...
- **`reference`** — entities imported by reference, `additionalProperties: true` for pass-through; eSAP does *not* redefine them. **USDM** study structure (`Study`, `StudyVersion`, `StudyDesign`, `Objective`, `Endpoint`, `Estimand`, `AnalysisPopulation`, `StudyIntervention`, `IntercurrentEvent`, `ScheduleTimeline`, `StudyDefinitionDocument`, …) — the **single source**; and the **ARS** `ReportingEvent` — the analysis-specification container (root `reportingEvents[]`).
- **`owned`** — eSAP-specific types: `Transformation` (abstract) and its concrete subclasses `Derivation` and `Analysis` (study-resolved instances, held in `ReportingEvent.analyses` / `.derivations`); structural twin-DSD types `DataStructure`, `DimensionEntry`, `MeasureEntry`, `SliceTemplate`, `SliceConstraint`, `SliceKey`; `MethodConfiguration`, `Configuration`, `PhraseInstance`, `ResolvedExpression`, `OutputClassConfig`; ICE-overlay types `IceAscertainment` (per-ICE strategy/timeline/ascertainment) and `IceHandling` (per-estimand handling); the executable-condition `OccurrenceCriterion` (a refinement of USDM `Condition`); confirmatory-inference types `Hypothesis` (forEstimand / testedBy; statement is prose) and `MultiplicityStrategy` (order or Bretz–Maurer graph over the hypotheses); typed enums `IchE9R1Strategy` and `AnalysisRole`.
- **`cross-reference`** — `TransformationRef`, `SmartPhraseRef`, `BiomedicalConceptRef`, `BiomedicalConceptPropertyRef`, `ResponseCodeRef` — typed FKs (id / path-shaped id) into the Transformation Library and USDM Biomedical Concepts, validated at build/exec time.

![Study eSAP schema (v0.5.0, Model B) — USDM v4.0 structural shell (green nodes), ACDC eSAP-owned overlay (amber nodes), and cross-reference stubs / code values (purple nodes). Force-directed layout showing Study Transformation as a full copy of its library TransformationTemplate (twin DSDs, slices, sliceKeys, methodConfigurations, validSmartPhrases) plus study overlays (sliceKeys[].value, configurationValues, hasPhraseInstance, resolvedExpression); Method stays referenced via usesMethod.](../images/study-esap-structure.png)

*Source: `model/linkML/esap_diagram.drawio` (canonical, editable in [draw.io](https://app.diagrams.net/)). Re-export with:* `drawio --export --format png --output images/study-esap-structure.png model/linkML/esap_diagram.drawio` *(also export SVG with `--format svg` if needed).*
...
| Analysis as a typed concept | `Analysis is_a Method` with `analysisReason` / `analysisPurpose` / `applicableWhen` / `inputData` | `Analysis is_a Transformation`, lives in an ARS `ReportingEvent`, plus `addressesEstimand` (USDM bridge) / `analysisRole` / `summaryOutput` (an output name within `usesMethod`) |
| Estimand modelling | No first-class Estimand; estimand information lives in narrative slots (`purpose`, `analysisReason`) | First-class **single-source** `Estimand` (USDM) with typed ICH E9(R1) attributes (`variableOfInterest`, `analysisPopulation`, `interventions`, `intercurrentEvents`); the eSAP's analyses/handling *reference* the estimand (they are not nested in it) |
| ICH E9(R1) strategy | Free-text or coded under `WhereClause` | Typed enum `IchE9R1Strategy` with five members on `IceAscertainment` (study-default) + reified `IceHandling` for per-estimand overrides |
| Intercurrent events | Not modelled directly | Pure single-source `IntercurrentEvent` (USDM); the eSAP-owned `IceAscertainment` references it and carries `hasScheduleTimeline` + `ascertainedBy` (`OccurrenceCriterion(s)` or `TransformationRef`) |
