...
12. **`modelInteractions` resolution and legality.** For each `inputDataStructure.modelInteractions[]` item (the per-use interaction terms selected onto the model RHS):
    ...
    - `operands` count ≥ 2.
    - There is no `kind` discriminator: a `ModelInteraction` IS an interaction (the `:` operator is the only selectable composite term in the current method library). Nesting / stratification (the `|` partition) is method-intrinsic and not modelled here.
13. **Interaction / output-term coherence (advisory, non-blocking).** For each `outputDataStructure.measures[]` item whose bound method-output declares `sub_types[]` with a multi-kind `formula_term_kinds` that *forms an allowed interaction production* (e.g. `M.MMRM`'s `group_contrast_at_time` → `["fixed_effect", "repeated_factor"]`): if the corresponding interaction is **absent** from `inputDataStructure.modelInteractions[]`, the validator emits a warning. That sub-type's by-cell breakdown presupposes an interaction the model does not fit, so the crossed-factor results collapse to additive (e.g. a "treatment difference *at each visit*" contrast would be constant across visits — the "at visit" qualifier becomes vacuous). This mirrors rule 9's non-blocking style: it flags a likely-wrong omission without forbidding the legitimate case where an interaction is intentionally excluded (the additive cell means are still *computable*, just not *informative*). To make a given breakdown blocking, a method MAY set an optional `sub_types[].requires_interaction: true` flag — keeping the statistical judgment on the method (where the model knowledge lives) rather than hard-coded in the validator.
